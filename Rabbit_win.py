...
class RabbitWin(QWidget):
    ...
    def loadfile_fn(self, filename, nb_lines_header):
        try:
            with open(filename, 'r') as file:
                data = []
                i = 0
                for line in file:
                    if i < nb_lines_header:
            ...
            print('ValueError: error in loadfile_fn')
    ...
    def importdata_lr(self) -> None:
        ...
        if (data_filename):
            ...
            fdir = fdir + '/'
            #flist = glob(fdir + '*delay_0*') # Labview 2013 program
            #flist = glob(fdir + '*1DViewer00Q.dat') # Labview 2016 program
            ...
            if (len(flist) != 0):
                ...
                try:
                    #data = np.loadtxt(data_filename, unpack=True) # for .txt data files only
                    data = self.loadfile_fn(data_filename, cts.nb_lines_header)
                except ValueError:  # if we don't choose a correct file but in the right directory
                    #data = np.loadtxt(flist[0], unpack=True) # for .txt data files only
                    data = self.loadfile_fn(flist[0], cts.nb_lines_header)


                self.tof = data[:, 0]
                self.toflength = data.shape[0]
                self.tof = self.tof[::-1]
                self.data_tof = []
                cts.rabbit_mat = []
                ...
                for fn in flist:
                    ...
                    #data = np.loadtxt(fn, unpack=True) # for .txt data files only
                    if cts.minus_sign:
                        data = -1 * self.loadfile_fn(fn, cts.nb_lines_header)
                    else:
                        data = self.loadfile_fn(fn, cts.nb_lines_header)
                    #counts = data[1, :]  - data[1, 300:600].mean() # with np.loadtxt()
                    counts = data[:, 1] - data[300:600, 1].mean() # background substraction
